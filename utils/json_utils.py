...
def _decode_json(line, path, line_number):
    try:
        ...
        custom_error_message = (
            f"Error parsing JSON on line {line_number}: {e.msg} at"
            f" {path}:{line_number}:{e.colno}, line: {line}"
        )
...
def append_jsonl(
    data: Any, file_path: str, ensure_ascii: bool = False, **kwargs: Any
) -> None:
    """
...
class EnhancedJSONEncoder(json.JSONEncoder):
    def __init__(self, exclude_keys: list[str] | None = None, **kwargs: Any):
